...
pytestmark = pytest.mark.skipif(
    not os.getenv("PLATFORM_GROQ_API_KEY"), reason="no PLATFORM_GROQ_API_KEY"
)


@pytest_asyncio.fixture
async def seed_groq(boss_user, db_pool):
    async with db_pool.acquire() as c:
        mid = await c.fetchval(
            ...
            INSERT INTO models (name, provider, endpoint_kind, base_url, tier, ctx_max,
                                ...
                                is_platform_default)
            VALUES ('llama-3.3-70b-versatile','groq','openai_compat',
                    'https://api.groq.com/openai/v1','fast',128000,
                    '["text","tools"]'::jsonb, 0.59, 0.79, TRUE)
            RETURNING id
            """
        )
        await c.execute(
            "UPDATE users SET fast_model_id=$1 WHERE id=$2", mid, boss_user["id"]
        )
        await c.execute(
            """
            INSERT INTO llm_routes (feature, target_tier, fallback_chain, weight)
            VALUES ('group_followup_decision','fast','[]'::jsonb, 100)
...
@pytest.mark.asyncio
async def test_complete_llama_fast(db_pool, boss_user, seed_groq):
    ...
    req = LLMRequest(
        feature="group_followup_decision",
