...
function MessageList({
  ...
  if (messages.length === 0)
    return <p className="text-sm text-muted-foreground p-4">Chưa có tin nhắn.</p>;
  ...
  for (const m of messages) {
    ...
    if (day !== lastDay) {
      rows.push({ kind: 'day', label: dayLabel(m.ts), key: `day-${day}` });
      lastDay = day;
    }
    ...
    <div className="space-y-2 p-1">
      {nextBefore && (
        <div className="text-center">
          ...
          >
            {loading ? 'Đang tải…' : 'Tải tin cũ hơn'}
          </Button>
        </div>
      )}
      {rows.map((row) =>
          ...
          >
            ...
            >
              <p className="text-[11px] font-medium text-muted-foreground mb-0.5">
                {row.m.direction === 'out' ? 'Bot' : row.m.sender_name ?? 'Không rõ'}
                <span className="font-normal ml-2">
                  {new Date(row.m.ts).toLocaleTimeString('vi-VN', {
                    hour: '2-digit',
                    minute: '2-digit',
                  })}
                </span>
  ...
  if (!convs.data || convs.data.length === 0)
    return (
      <p className="text-sm text-muted-foreground">
        Boss này chưa có hội thoại nào.
      </p>
    );
            ...
            >
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                <span className="text-[11px] text-muted-foreground">
                  {c.msg_count.toLocaleString('vi-VN')} tin
                  {c.last_ts &&
                    ` · ${new Date(c.last_ts).toLocaleDateString('vi-VN')}`}
                </span>
              </div>
            </button>
          );
