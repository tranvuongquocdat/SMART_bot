...
## Lark Base — Xem dữ liệu trực tiếp

Toàn bộ dữ liệu (task, nhân sự, dự án, ý tưởng) được lưu trên **Lark Base** — bạn có thể mở Lark để xem, lọc, sắp xếp bằng giao diện bảng tính.

### Cách truy cập

Sau khi tạo workspace, bot gửi link Lark Base cho bạn:
```
Lark Base: https://larksuite.com/base/xxx
```
Mở link → đăng nhập Lark → xem toàn bộ dữ liệu.

### Cấu trúc dữ liệu

Mỗi công ty/sếp có 1 Lark Base riêng tên `{Tên công ty} - AI Secretary`, gồm 4 bảng:

| Bảng | Nội dung | Các cột chính |
|------|----------|---------------|
| **People** | Danh sách nhân sự | Tên, Tên gọi, Chat ID, Type, Nhóm, Vai trò, Kỹ năng, SĐT, Ghi chú |
| **Tasks** | Công việc | Tên task, Assignee, Deadline, Priority, Status, Project, Location |
| **Projects** | Dự án | Tên dự án, Mô tả, Người phụ trách, Thành viên, Deadline, Trạng thái |
| **Ideas** | Ý tưởng | Nội dung, Tags, Người tạo, Project |

### Bot tạo/sửa/xóa — Lark để xem

- **Mọi thao tác CRUD** (tạo, sửa, xóa task/người/dự án) đều thông qua bot trên Telegram
- **Lark Base** là nơi xem dữ liệu dạng bảng, lọc, sắp xếp, xuất Excel
- Bạn **có thể sửa trực tiếp trên Lark**, nhưng bot sẽ không biết thay đổi đó (bot chỉ đọc khi cần)
- Nên ưu tiên thao tác qua bot để dữ liệu đồng bộ (đặc biệt với Tasks — bot cũng lưu vào hệ thống tìm kiếm)

### Khi nào dữ liệu được tạo

| Sự kiện | Dữ liệu tạo trên Lark |
|---------|----------------------|
| Sếp onboard | Base + 4 bảng + record sếp trong People |
| Thành viên/đối tác onboard | Record mới trong People |
| Sếp giao task | Record mới trong Tasks |
| Sếp tạo dự án | Record mới trong Projects |
| Sếp lưu ý tưởng | Record mới trong Ideas |
| Sếp thêm người | Record mới trong People |
